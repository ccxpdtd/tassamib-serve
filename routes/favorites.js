...
const handleFavorites = (results) => {
  const video = results.filter(item => item.type === 'video')
  const album = results.filter(item => item.type === 'album')
  const show = results.filter(item => item.type === 'show')
  const game = results.filter(item => item.type === 'game')
  const recently_watching = results.filter(item => item.type === 'recently_watching')
  const recently_listening = results.filter(item => item.type === 'recently_listening')
  return {
    video,
    album,
    show,
...
// 读取所有文章接口
router.get('/get_myFavorites', (req, res) => {

  const sql = `SELECT * FROM my_favorites 
  ...
  db.query(sql, (err, results) => {
    ...
    const favorites = handleFavorites(results)
    res.send({
      code: 200,
      msg: '获取喜欢列表成功',
      favorites
